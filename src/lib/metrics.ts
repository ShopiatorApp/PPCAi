...
// Ngram analysis functions
export function generateNgrams(term: string, n: number): string[] {
...
export function analyzeNgrams(terms: SearchTermMetric[], n: number): NgramMetric[] {
  const ngramMap = new Map<string, NgramMetric>();
  
  terms.forEach(term => {
    const ngrams = generateNgrams(term.search_term, n);
    ...
    ngrams.forEach(ngram => {
      ...
      const metric = ngramMap.get(ngram)!;
      metric.count++;
      metric.totalCost += term.cost;
      metric.totalClicks += term.clicks;
      metric.totalImpr += term.impr;
      metric.totalConv += term.conv;
      metric.totalValue += term.value;
    });
  });
